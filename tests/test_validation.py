...
def test_api_overview_renders_html_for_browsers(monkeypatch):
    monkeypatch.setenv("NETBOX_URL", "http://10.254.0.15:8000")
    monkeypatch.setenv("NETBOX_TOKEN", "Bearer test-token")
    monkeypatch.setenv("SYNC_API_KEY", "test-api-key")
    get_settings.cache_clear()
    ...
    with TestClient(app) as client:
        response = client.get("/api/overview", headers={"accept": "text/html,application/xhtml+xml"})

    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/html")
    assert "Central de rede" in response.text
    assert "Dashboard v" in response.text


def test_api_overview_keeps_json_for_clients(monkeypatch):
    monkeypatch.setenv("NETBOX_URL", "http://10.254.0.15:8000")
    monkeypatch.setenv("NETBOX_TOKEN", "Bearer test-token")
    monkeypatch.setenv("SYNC_API_KEY", "test-api-key")
    get_settings.cache_clear()
    ...
    with TestClient(app) as client:
        response = client.get("/api/overview", headers={"accept": "application/json"})
...
def test_api_config_renders_html_for_browsers(monkeypatch):
    monkeypatch.setenv("NETBOX_URL", "http://10.254.0.15:8000")
    monkeypatch.setenv("NETBOX_TOKEN", "Bearer test-token")
    monkeypatch.setenv("SYNC_API_KEY", "test-api-key")
    get_settings.cache_clear()
    ...
    with TestClient(app) as client:
        response = client.get("/api/config", headers={"accept": "text/html,application/xhtml+xml"})

    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/html")
    assert "Configurações da API" in response.text
    assert "Conectores mascarados" in response.text


def test_api_config_keeps_json_for_clients(monkeypatch):
    monkeypatch.setenv("NETBOX_URL", "http://10.254.0.15:8000")
    monkeypatch.setenv("NETBOX_TOKEN", "Bearer test-token")
    monkeypatch.setenv("SYNC_API_KEY", "test-api-key")
    get_settings.cache_clear()
    ...
    assert response.headers["content-type"].startswith("application/json")
    payload = response.json()
    assert "netbox" in payload
    assert "zabbix" in payload
    ...
    probe_state = {
        "devices": [
            ...
            },
            {
                "ip": "10.0.0.41",
                "sys_name": "SW-EDGE-01",
                "snmp_mac_address": "AA:BB:CC:DD:EE:FF",
                "ports": [
                    {"index": "24", "name": "ge-0/0/24", "mac_address": "AA:BB:CC:DD:EE:FF"},
                ],
                "bridge_port_map": [
                    {"bridge_port": "24", "if_index": "24"},
                ],
                "bridge_fdb": [
                    {"mac_address": "D8:44:89:52:A6:B1", "bridge_port": "24"},
                ],
            },
        ]
    }
...
def test_bridge_fdb_mac_records_require_reciprocal_signal(monkeypatch):
    monkeypatch.setenv("NETBOX_URL", "http://10.254.0.15:8000")
    monkeypatch.setenv("NETBOX_TOKEN", "Bearer test-token")
    monkeypatch.setenv("ZABBIX_URL", "http://10.254.0.15/api_jsonrpc.php")
    monkeypatch.setenv("ZABBIX_TOKEN", "Bearer zabbix-token")
    ...
    probe_state = {
        "devices": [
            ...
            },
            {
                ...
                "bridge_fdb": [
                    {"mac_address": "11:22:33:44:55:66", "bridge_port": "24"},
                ],
            },
        ]
    }
    ...
    assert all(not (edge["edge_type"] == "bridge-link" and edge["target"] == "102") for edge in edges)
